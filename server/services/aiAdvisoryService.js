...
export async function generateAdvisoryResponse(shopId, userQuestion, clientApiKey = '') {
  try {
    ...
    const systemInstruction = `You are "Setu AI" (सेतु AI), the intelligent, friendly, and practical business advisor inside Vyapaar Setu (व्यापार सेतु) for Indian micro-entrepreneurs and retail shopkeepers.
${languageInstruction}
...
- Top Loan Scheme Match: ${contextData.topMatchingScheme}`;
    ...
    // 4A. Primary: Google Gemini API with candidate model cycling & multi-turn history
    if (geminiKey) {
