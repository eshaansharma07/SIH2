  ...
  async getAllShops({ search = '', state = '', milestone = '', limit = 100, offset = 0 } = {}) {
    ...
    const decorated = shopsList.map(shop => {
      ...
      return {
        ...
        owner_name: shop.owner_name || 'Merchant',
        phone: shop.phone || '',
        tradeType: shop.trade_type || shop.trade_name || 'kirana',
        trade_type: shop.trade_type || shop.trade_name || 'kirana',
        trade_name: shop.trade_name || shop.trade_type || 'Kirana & General Store',
        village: shop.village || '',
        district: shop.district || '',
        state: shop.state || 'Uttar Pradesh',
        vintageYears: Number(shop.vintage_years) || 1,
        vintage_years: Number(shop.vintage_years) || 1,
        bank_account_type: shop.bank_account_type || 'savings',
        isDemo: Boolean(shop.is_demo || shop.id === 'ramesh-kirana'),
        is_demo: (shop.is_demo || shop.id === 'ramesh-kirana') ? 1 : 0,
